...
export function StatusBadge({ label, count, color, letter }: { label: string; count: number; color: string; letter: string }) {
  ...
  return (
    <div className="flex items-center gap-2 text-ui-body">
      <div className="w-6 h-6 rounded flex items-center justify-center text-[length:var(--font-size-xs)] font-bold shrink-0" style={{ backgroundColor: bg, color }}>
        {letter}
      </div>
...
export function FlowStep({ n, done, children }: { n: number; done: boolean; children: React.ReactNode }) {
  return (
    <li className="flex items-start gap-3">
      <span className={cn('shrink-0 w-5 h-5 rounded-full flex items-center justify-center text-[length:var(--font-size-xs)] font-bold', done ? 'bg-secondary text-bg-base' : 'bg-border-subtle text-text-secondary')}>
        {done ? '✓' : n}
      </span>
