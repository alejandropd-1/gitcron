...
function DetailField({
  ...
  return (
    <div className="min-w-0 rounded border border-border-subtle/15 bg-bg-surface/55 px-3 py-2">
      <div className="text-[length:var(--font-size-2xs)] font-bold uppercase tracking-wider text-text-secondary/80">{label}</div>
      <div
        className={cn(
          'mt-1 text-[length:var(--font-size-xs)] leading-relaxed text-text-primary/90',
...
function DecisionBadge({ kind, lang }: { kind: DecisionKind; lang: Lang }) {
  ...
  return (
    <span
      className="rounded border px-1.5 py-0.5 text-[length:var(--font-size-2xs)] font-bold uppercase tracking-wider"
      style={{
        color,
    ...
    <div className="flex flex-col gap-3 px-4 py-2.5 font-mono">
      <div className="flex items-center justify-between gap-3 border-b border-primary/15 pb-2">
        <button
          type="button"
          onClick={onBack}
          className="rounded border border-primary/25 bg-primary/10 px-2.5 py-1 text-[length:var(--font-size-2xs)] font-bold uppercase tracking-wider text-primary transition-colors hover:border-primary/55 hover:bg-primary/15"
        >
          ← {translate('predictionDetail.back', lang)}
        </button>
        <div className="min-w-0 text-right">
          <div className="text-[length:var(--font-size-2xs)] font-bold uppercase tracking-wider text-text-secondary/75">
            {translate('predictionDetail.title', lang)}
          </div>
          <div className="truncate text-[length:var(--font-size-xs)] font-bold text-text-primary">{formatValue(branch.message)}</div>
        </div>
      </div>
      ...
      <section className="overflow-hidden rounded border border-border-subtle/15 bg-bg-surface/55">
        <header className="border-b border-border-subtle/15 px-3 py-2 text-[length:var(--font-size-2xs)] font-bold uppercase tracking-wider text-text-secondary">
        ...
        {sortedDecisions.length > 0 ? (
              ...
              return (
                <li key={decision.id} className="flex flex-col gap-1.5 px-3 py-2">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <time className="text-[length:var(--font-size-xs)] text-text-secondary/75" dateTime={decision.decidedAt}>
          ...
          <p className="px-3 py-2 text-[length:var(--font-size-xs)] italic text-text-secondary/70">
