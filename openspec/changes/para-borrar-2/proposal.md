...
Este cambio se crea a pedido de Alejandro con el objetivo explícito «otra prueba para borrar»: no
responde a un problema medido en la aplicación ni a un pedido de producto. Sirve como cambio de
sonda —para probar el ciclo OpenSpec (crear, ver el recorrido, archivar o retirar) desde GitCron sin
afectar ninguna capability real— y se espera que se retire o se borre sin consolidar specs.
...
- Sin impacto en código, dependencias ni configuración. Sólo agrega los artefactos de planificación
  de este directorio (`openspec/changes/para-borrar-2/`).
